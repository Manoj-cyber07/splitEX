...
// ==========================================
const SOROBAN_CONTRACT_ID = 'CAHORHFAW4MAIPWM4EDOWST3IP4MGPARHZ4EMXMUPYUBMAYO76JDDHAB';
...
const firebaseConfig = {
  apiKey: "AIzaSyDU1nsLlKMhqjdRY_qrp4qKawyZJt8TNq0",
  authDomain: "splitex-fd564.firebaseapp.com",
  projectId: "splitex-fd564",
  storageBucket: "splitex-fd564.firebasestorage.app",
  messagingSenderId: "681803534542",
  appId: "1:681803534542:web:9f538c9a73b61a69a2409f",
  measurementId: "G-QSSPG4V3C6"
};
...
// ==========================================
// 1b. SOROBAN CONTRACT INTEGRATION
// Real on-chain calls to the deployed splitEX escrow contract.
// These run *in addition to* the Firebase + direct-payment flow, so the
// app keeps working even if a contract call fails (best-effort, non-blocking).
// ==========================================
const SOROBAN_RPC_URL = 'https://soroban-testnet.stellar.org';

const sdk = () => window.StellarSdk;
const rpcLib = () => { const S = sdk(); return S.rpc || S.SorobanRpc; };
const rpcServer = () => new (rpcLib().Server)(SOROBAN_RPC_URL, { allowHttp: false });
const netPassphrase = (network) => network === 'mainnet' ? sdk().Networks.PUBLIC : sdk().Networks.TESTNET;
// Soroban Symbol: alphanumeric/underscore, max 32 chars.
const toSymbol = (str) => {
  let s = (str || 'Split').replace(/[^a-zA-Z0-9_]/g, '');
  return (s || 'Split').substring(0, 32);
};
const isSimError = (r) => {
  const A = rpcLib().Api;
  return A && A.isSimulationError ? A.isSimulationError(r) : !!r.error;
};

// Read-only contract call (simulation only — no wallet, no fees).
async function readContract(method, args, sourcePubKey, network) {
  const S = sdk();
  const server = rpcServer();
  const contract = new S.Contract(SOROBAN_CONTRACT_ID);
  const account = await server.getAccount(sourcePubKey);
  const tx = new S.TransactionBuilder(account, { fee: '2000000', networkPassphrase: netPassphrase(network) })
    .addOperation(contract.call(method, ...args)).setTimeout(60).build();
  const sim = await server.simulateTransaction(tx);
  if (isSimError(sim)) throw new Error('Simulation failed: ' + (sim.error || method));
  return sim.result && sim.result.retval ? S.scValToNative(sim.result.retval) : null;
}

// Sign a prepared transaction XDR with the user's connected wallet (Albedo/Freighter).
async function signWithWallet(txXdr, network) {
  const S = sdk();
  const provider = sessionStorage.getItem('splitex_wallet_provider');
  const passphrase = netPassphrase(network);
  if (provider === 'albedo') {
    const res = await window.albedo.tx({ xdr: txXdr, network: network, pubkey: sessionStorage.getItem('splitex_full_key') });
    return S.TransactionBuilder.fromXDR(res.signed_envelope_xdr || res.xdr, passphrase);
  }
  const api = window.freighterApi;
  try { if (api && typeof api.requestAccess === 'function') await api.requestAccess(); } catch (e) {}
  const signedRes = await api.signTransaction(txXdr, { network: network === 'mainnet' ? 'PUBLIC' : 'TESTNET', networkPassphrase: passphrase });
  const signedXdr = typeof signedRes === 'string' ? signedRes : (signedRes.signedTxXdr || signedRes.signedXDR || signedRes);
  return S.TransactionBuilder.fromXDR(signedXdr, passphrase);
}

// Write contract call: build, simulate (capture return), assemble, sign, send, poll.
async function invokeContract(method, args, sourcePubKey, network) {
  const S = sdk();
  const server = rpcServer();
  const contract = new S.Contract(SOROBAN_CONTRACT_ID);
  const account = await server.getAccount(sourcePubKey);
  let tx = new S.TransactionBuilder(account, { fee: '2000000', networkPassphrase: netPassphrase(network) })
    .addOperation(contract.call(method, ...args)).setTimeout(120).build();

  const sim = await server.simulateTransaction(tx);
  if (isSimError(sim)) throw new Error('Simulation failed: ' + (sim.error || method));
  let retval = null;
  try { if (sim.result && sim.result.retval) retval = S.scValToNative(sim.result.retval); } catch (e) {}

  const prepared = rpcLib().assembleTransaction(tx, sim).build();
  const signedTx = await signWithWallet(prepared.toXDR(), network);
  const send = await server.sendTransaction(signedTx);
  if (send.status === 'ERROR') {
    let reason = '';
    try { reason = send.errorResultXdr || (send.errorResult ? send.errorResult.toString() : ''); } catch (e) {}
    throw new Error('RPC rejected the transaction (' + (reason || 'check wallet network = TESTNET') + ')');
  }
  // Poll status via raw JSON-RPC. We deliberately avoid the SDK's getTransaction()
  // here because it decodes the result metadata, and Protocol 23 testnet returns
  // TransactionMetaV4 which older SDK builds can't parse ("Bad union switch: 4").
  // The return value we need was already captured from the simulation above.
  const status = await pollTxStatus(send.hash);
  if (status !== 'SUCCESS') throw new Error('Transaction did not succeed on-chain (status: ' + status + ', hash: ' + send.hash + ')');
  return { hash: send.hash, retval };
}

// Poll a Soroban transaction's status via raw JSON-RPC (no XDR/metadata decoding).
async function pollTxStatus(hash) {
  for (let i = 0; i < 25; i++) {
    try {
      ...
      const status = json && json.result ? json.result.status : 'NOT_FOUND';
      if (status && status !== 'NOT_FOUND') return status; // SUCCESS / FAILED
...
// Create the split on-chain. Returns { splitId, hash }. Signed by the creator's wallet.
async function contractCreateSplit(split, network) {
  const S = sdk();
  const creator = (split.creator || '').trim().toUpperCase();
  const friends = (split.friends || []).map(f => (f.address || '').trim().toUpperCase()).filter(Boolean);
  const n = friends.length;
  if (!creator || n === 0) throw new Error('Missing creator or participants');
  const base = Math.floor(100 / n);
  const shares = new Array(n).fill(base);
  shares[n - 1] += 100 - base * n; // make percentages sum to exactly 100
  const shareStroops = BigInt(Math.round(split.share * 1e7));
  const totalStroops = shareStroops * BigInt(n);
  const token = S.Asset.native().contractId(netPassphrase(network));
  const args = [
    S.nativeToScVal(creator, { type: 'address' }),
    S.nativeToScVal(toSymbol(split.name), { type: 'symbol' }),
    S.nativeToScVal(totalStroops, { type: 'i128' }),
    S.nativeToScVal(token, { type: 'address' }),
    S.xdr.ScVal.scvVec(friends.map(a => new S.Address(a).toScVal())),
    S.xdr.ScVal.scvVec(shares.map(s => S.nativeToScVal(s, { type: 'u32' })))
  ];
  const res = await invokeContract('create_split', args, creator, network);
  return { splitId: res.retval != null ? res.retval.toString() : null, hash: res.hash };
}

// Record a participant's deposit on-chain. Deposits exactly the remaining owed amount
// (read from the contract) to avoid rounding overflow. Returns the tx hash.
async function contractDepositShare(onchainSplitId, payerPubKey, network) {
  const S = sdk();
  const payer = (payerPubKey || '').trim().toUpperCase();
  const idScVal = S.nativeToScVal(BigInt(onchainSplitId), { type: 'u64' });
  const share = await readContract('get_participant_share', [idScVal, S.nativeToScVal(payer, { type: 'address' })], payer, network);
  const remaining = BigInt(share.amount_to_pay) - BigInt(share.amount_paid);
  if (remaining <= 0n) return { hash: null, alreadyPaid: true };
  const args = [
    S.nativeToScVal(payer, { type: 'address' }),
    S.nativeToScVal(BigInt(onchainSplitId), { type: 'u64' }),
    S.nativeToScVal(remaining, { type: 'i128' })
  ];
  const res = await invokeContract('deposit_share', args, payer, network);
  return { hash: res.hash };
}

// Mark a fully-paid split as settled on-chain. Returns the tx hash.
async function contractSettleSplit(onchainSplitId, sourcePubKey, network) {
  const S = sdk();
  const args = [S.nativeToScVal(BigInt(onchainSplitId), { type: 'u64' })];
  const res = await invokeContract('settle_split', args, (sourcePubKey || '').trim().toUpperCase(), network);
  return { hash: res.hash };
}
  ...
  return (
    <main className="relative z-10 pt-32 px-4 pb-24 w-full max-w-md mx-auto flex flex-col items-center animate-[fadeIn_0.3s_ease-out]">
      <div className="w-full premium-card p-8 rounded-3xl relative z-10 shadow-[0_0_50px_rgba(59,130,246,0.15)]">
        ...
        <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#1e293b] to-[#0f172a] border border-white/10 flex items-center justify-center font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400 mx-auto mb-6 shadow-inner">SE</div>
...
const NewSplitModal = ({ isOpen, onClose, onSplitCreated }) => {
  ...
  const handleCreate = () => {
    ...
    const pubKey = sessionStorage.getItem('splitex_full_key') || '';
...
const SendModal = ({ isOpen, onClose, network, prefillData, onPaymentSuccess }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const provider = sessionStorage.getItem('splitex_wallet_provider');
      let pubKey = sessionStorage.getItem('splitex_full_key');
      const passphrase = network === 'mainnet' ? window.StellarSdk.Networks.PUBLIC : window.StellarSdk.Networks.TESTNET;
      const netName = network === 'mainnet' ? 'PUBLIC' : 'TESTNET';
      const cleanAmount = parseFloat(amount).toFixed(7).replace(/\.?0+$/, '');
      let generatedTxHash = null;

      if (provider === 'albedo') {
        const res = await window.albedo.pay({ amount: cleanAmount, destination: cleanAddress, network, submit: true });
        generatedTxHash = res.tx_hash || (res.result && res.result.hash) || null;
        if (!generatedTxHash) throw new Error("Albedo did not return a transaction hash — the payment was not submitted. (If the recipient is a brand-new testnet account, it must be funded first.)");
      } else {
        const api = window.freighterApi;
        if (!api) throw new Error("Freighter wallet not detected.");
        try { if (typeof api.requestAccess === 'function') await api.requestAccess(); } catch (e) { }
        try {
          const freshKeyRes = await api.getPublicKey();
          const fk = typeof freshKeyRes === 'object' ? (freshKeyRes.publicKey || freshKeyRes.address) : freshKeyRes;
          if (fk) pubKey = fk;
        } catch (e) { }

        const HorizonServer = window.StellarSdk.Horizon ? window.StellarSdk.Horizon.Server : window.StellarSdk.Server;
        const server = new HorizonServer(network === 'mainnet' ? 'https://horizon.stellar.org' : 'https://horizon-testnet.stellar.org');
        const account = await server.loadAccount(pubKey);
        const tx = new window.StellarSdk.TransactionBuilder(account, { fee: "100000", networkPassphrase: passphrase })
        ...
        const signedRes = await api.signTransaction(tx.toXDR(), { network: netName, networkPassphrase: passphrase });
        const signedXdr = typeof signedRes === 'string' ? signedRes : (signedRes.signedTxXdr || signedRes.signedXDR || signedRes.signedTransaction || signedRes);
        if (!signedXdr || typeof signedXdr !== 'string') throw new Error("Wallet did not return a signed transaction.");
        const signedTx = window.StellarSdk.TransactionBuilder.fromXDR(signedXdr, passphrase);
        const submitRes = await server.submitTransaction(signedTx);
        generatedTxHash = submitRes.hash;
      }

      if (!generatedTxHash) throw new Error("No transaction hash was returned — payment not confirmed.");
      setStatus('success');
      if (onPaymentSuccess) onPaymentSuccess(generatedTxHash);
      setTimeout(() => { onClose(); setStatus('idle'); setAddress(''); setAmount(''); }, 2000);
    } catch (e) {
      setStatus('error');
      let detail = e && e.message ? e.message : "Network rejected.";
      const codes = e && e.response && e.response.data && e.response.data.extras && e.response.data.extras.result_codes;
      if (codes) detail = "Stellar error: " + JSON.stringify(codes);
...
const Dashboard = ({ network, setNetwork, splits, setSplits, userProfile }) => {
  ...
  const currentUserKey = (sessionStorage.getItem('splitex_full_key') || '').trim().toUpperCase();
  ...
  const handleCreateSplit = async (newSplit) => {
    // Record the split on-chain via the Soroban escrow contract (best-effort, non-blocking).
    try {
      if (window.StellarSdk && SOROBAN_CONTRACT_ID && !SOROBAN_CONTRACT_ID.startsWith('YOUR_')) {
        const onchain = await contractCreateSplit(newSplit, network);
        newSplit.onchainSplitId = onchain.splitId;
        newSplit.contractTxHash = onchain.hash;
      }
    } catch (e) {
      console.error('On-chain create_split failed:', e);
      alert('The split was saved, but the on-chain contract call did not complete:\n' + (e.message || e));
    }
    if (db) {
      try {
      ...
      sessionStorage.setItem('splitex_user_splits', JSON.stringify(updatedSplits));
    }
  };
  ...
  const handlePaymentSuccess = async (txHash) => {
    ...
    let onchainSplitId = null;
    let allPaidFlag = false;
    ...
    // Mirror the deposit (and settlement) on-chain via the escrow contract (best-effort).
    if (onchainSplitId && window.StellarSdk) {
      try {
        await contractDepositShare(onchainSplitId, currentUserKey, network);
        if (allPaidFlag) {
          try { await contractSettleSplit(onchainSplitId, currentUserKey, network); }
          catch (e) { console.error('On-chain settle_split failed:', e); }
        }
      } catch (e) { console.error('On-chain deposit_share failed:', e); }
    }
...
function MainApp() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && !window.StellarSdk) {
      const script = document.createElement('script'); script.src = 'https://cdn.jsdelivr.net/npm/@stellar/stellar-sdk@13.3.0/dist/stellar-sdk.min.js'; document.head.appendChild(script);
    }
    ...
    if (auth && db) {
      ...
      const fullKey = (sessionStorage.getItem('splitex_full_key') || '').trim().toUpperCase();
      if (fullKey) {
        let createdSplits = [];
        let inboxSplits = [];
      ...
      setIsLoadingAuth(false);
      const savedUser = sessionStorage.getItem('splitex_user_profile');
      if (savedUser) setUserProfile(JSON.parse(savedUser));
      const savedSplits = sessionStorage.getItem('splitex_user_splits');
      if (savedSplits) setSplits(JSON.parse(savedSplits));
      const handleStorage = (e) => { if (e.key === 'splitex_user_splits' && e.newValue) setSplits(JSON.parse(e.newValue)); };
      window.addEventListener('storage', handleStorage);
      return () => window.removeEventListener('storage', handleStorage);
    }

    const savedAddress = sessionStorage.getItem('splitex_wallet_address');
  ...
  const handleProfileUpdate = async (p) => {
    setUserProfile(p);
    sessionStorage.setItem('splitex_user_profile', JSON.stringify(p));
  ...
  const connectWallet = async (provider) => {
    try {
      let pubKey = '';
      if (provider === 'albedo') {
        const res = await window.albedo.publicKey({ token: 'splitex_auth' }); pubKey = res.pubkey;
      } else {
      ...
      sessionStorage.setItem('splitex_full_key', pubKey);
      sessionStorage.setItem('splitex_wallet_address', pubKey.substring(0, 4) + '...' + pubKey.substring(pubKey.length - 4));
      sessionStorage.setItem('splitex_wallet_provider', provider);
  ...
  if (isLoadingAuth) return <div className="min-h-screen bg-[#03050a] flex items-center justify-center text-blue-400 font-bold tracking-widest uppercase animate-pulse">Loading splitEX...</div>;
        ...
        <nav className="fixed top-4 inset-x-0 px-4 sm:px-6 z-40 flex justify-center w-full pointer-events-none">
          <div className="w-full max-w-6xl premium-card rounded-2xl px-4 py-3 flex justify-between items-center pointer-events-auto shadow-2xl backdrop-blur-3xl bg-[#03050a]/50">
            <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setCurrentView('landing')}>
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#1e293b] to-[#0f172a] border border-white/10 flex items-center justify-center font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400 shadow-inner">SE</div>
              <h1 className="text-2xl font-black hidden sm:block">splitEX</h1>
            </div>
          ...
          <div className="fixed inset-0 z-[90] flex items-center justify-center px-4 bg-[#03050a]/90 backdrop-blur-md animate-[fadeIn_0.2s_ease-out]">
            <div className="w-full max-w-sm premium-card rounded-3xl p-6 text-center relative shadow-[0_0_50px_rgba(239,68,68,0.15)]">
              ...
              <div className="flex gap-4">
                ...
                <button onClick={() => {
                  sessionStorage.removeItem('splitex_wallet_address');
                  sessionStorage.removeItem('splitex_full_key');
                  sessionStorage.removeItem('splitex_wallet_provider');
